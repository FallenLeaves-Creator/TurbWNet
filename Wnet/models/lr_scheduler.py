...
class CosineContinousAnnealingRestartCyclicLR(_LRScheduler):
    """ Cosine annealing with restarts learning rate scheme.
    An example of config:
    periods = [10, 10, 10, 10]
    restart_weights = [1, 0.5, 0.5, 0.5]
    ...
    def __init__(self,
        ...
        super(CosineContinousAnnealingRestartCyclicLR, self).__init__(optimizer, last_epoch)
    ...
    def get_lr(self):
        ...
        if idx==0:
            ...
            eta_min = self.eta_mins[idx]
            eta_max = self.eta_mins[idx-1]
            return [
                eta_min + current_weight * 0.5 * (eta_max - eta_min) *
                (1 + math.cos(math.pi * (
                    (self.last_epoch - nearest_restart) / current_period)))
            ]